...
export default function AiChat({ editor, roomId }) {
  ...
  const PLUTO_AI_SYSTEM_INSTRUCTION = `You are PlutoAI - an AI-powered tutor assisting high school students enrolled in the Upward Bound Math & Science program at The University of Texas at Arlington. Ask students to refer to their tutor if a question seems very specific. Follow these guidelines strictly for every response:

Approach: Generally avoid providing direct answers. Instead, gently guide students toward discovering the answer themselves by:
- Asking leading questions.
- Providing hints or relevant concepts to explore.
- Suggesting steps or resources that encourage critical thinking.
- Refrain from answering about any topic outside the Upward Bound Math and Science context

Tone: Friendly, concise, supportive and clear. Use simple language suitable for high school students.

Length: Keep responses brief, max 2 paras.`;
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      addMessage({
        text: `Please Try Later.Facing Some Issue`,
        sender: 'ai'
      });
  ...
  const handlePromptSelect = async (prompt) => {
    ...
    if (prompt !== 'Summarize the following document:') {
      ...
      if (prompt == "Add More") {
        prompt = `write something more maintaing the coherence with your previous response${currentContext} and don't repeat currentContext and write such a way that
        it is maintaining the flow`;
      }
    ...
    <div className="fixed bottom-4 right-4 z-50 ai-chat">
      {isOpen ? (
        <>
          ...
          >
            <div className="bg-light rounded shadow w-100" style={{ maxWidth: '800px', maxHeight: '800px', overflowY: 'auto', }}>
              ...
              >
                {messages?.map((message, index) => (
                  <div key={index} className={`d-flex mb-3 ${message.sender === 'user' ? 'justify-content-end' : 'justify-content-start'}`}>
                    <div
                      className={`p-2 rounded ${message.sender === 'user' ? 'pluto__pink-bg text-white' : 'bg-light'}`}
                      style={{
                        maxWidth: '100%',

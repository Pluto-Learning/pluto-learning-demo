...
import { fetchCourseInstructorSectionByInstructorId, fetchStudentCourseSectionDetailsById, fetchTableDesilsByUserId } from '@/app/api/crud'
...
export default function page() {
    ...
    const studentCombinedData = tableDesilsByUserId.map(item => {
    ...
    const getCourseInstructorSectionByInstructorId = async () => {
        try {
            ...
            setinstructorScheduleData(data);
        } catch (error) {
    ...
    const instructorEvents = instructorScheduleData.map(item => ({
        title: `${item.courseName} - ${item.sectionName}`,
        start: item.sectionStartTime,
        end: item.sectionEndTime,
        extendedProps: {
    ...
    console.log('combinedData: ', studentCombinedData)
                    ...
                    <div className="calendar-schedule">
                        <div className="row g-3">
                            ...
                            <div className="col-xl-8">
                                ...
                                <FullCalendarIo
                                    data={
                                        session?.user?.userType?.toLowerCase() === 'instructor'
                                            ? instructorEvents
                                            : session?.user?.userType?.toLowerCase() === 'student'
                                                ? studentCombinedData
                                                : session?.user?.userType?.toLowerCase() === 'admin'
                                                    ? [...instructorEvents, ...studentCombinedData]
                                                    : []
                                    }
                                />
